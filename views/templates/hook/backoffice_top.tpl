...
<div class="onboarding minimized">
	...
	<div class="panel steps">
		...
		<div id="onboarding-starter" class="hide">
			...
			<a href="#" class="btn btn-primary quick-start-button">
		...
		<div class="steps-list-container">
			<ul class="steps-list">
				<li {if $steps[1] == 1}class="checked"{/if}{if $current_step == 1}class="active"{/if}{if $current_step < 1}class="inactive"{/if}>
					...
					{else if $steps[1] == 1}
						<p class="desc">
							<a class="" href="{$link->getAdminLink('AdminThemes')|escape:'html':'UTF-8'}">
				...
				<li {if $steps[2] == 1}class="checked"{/if}{if $current_step == 2}class="active"{/if}{if $current_step < 2}class="inactive"{/if}>
					...
					{if $steps[2] == 0 && $current_step == 2}
						<p class="desc">
							{assign "onboardingstep2importcsv" $link->getAdminLink('AdminImport')|escape:'html':'UTF-8' }
							...
							<br><br>
							<a class="btn btn-primary" href="{$link->getAdminLink('AdminProducts')|escape:'html':'UTF-8'}&amp;addproduct">
							<i class="icon icon-book icon-lg"></i>
							{l s='Add products' mod='onboarding'}</a>
						</p>
					{else if $steps[2] == 1}
						<p class="desc">
							<a class="" href="{$link->getAdminLink('AdminProducts')|escape:'html':'UTF-8'}&amp;addproduct">{l s='Continue adding products' mod='onboarding'}</a>
						</p>
					{/if}
				</li>
				<li {if $steps[3] == 1}class="checked"{/if}{if $current_step == 3}class="active"{/if}{if $current_step < 3}class="inactive"{/if}>
					...
					{if $steps[3] == 0 && $current_step == 3}
						<p class="desc">
							...
							<br><br>
							<a class="btn btn-primary" href="{$link->getAdminLink('AdminPayment')|escape:'html':'UTF-8'}">
					...
					{else if $steps[3] == 1}
						<p class="desc">
							<a class="" href="{$link->getAdminLink('AdminPayment')|escape:'html':'UTF-8'}">{l s='Continue selecting payment methods' mod='onboarding'}</a>
						</p>
					{/if}
				</li>
				<li {if $steps[4] == 1}class="checked"{/if}{if $current_step == 4}class="active"{/if}{if $current_step < 4}class="inactive"{/if}>
					...
					<p class="desc">
						...
						<br>
						<br>
						<a class="btn btn-primary" href="{$link->getAdminLink('AdminCarriers')|escape:'html':'UTF-8'}&amp;onboarding_carrier">
					...
					{else if $steps[4] == 1}
						<p class="desc">
							<a class="" href="{$link->getAdminLink('AdminCarriers')|escape:'html':'UTF-8'}&amp;onboarding_carrier">{l s='Continue selecting shipping methods' mod='onboarding'}</a>
						</p>
					{/if}
				</li>
			</ul>
